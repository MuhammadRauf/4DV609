...
public interface FriendEvent extends Jsonable, AggregateEvent<FriendEvent> {
  ...
  public class UserCreated implements FriendEvent {
    ...
    public UserCreated(String userId, String name, String pw) {
    ...
    @JsonCreator
    private UserCreated(String userId, String name, String pw, Optional<Instant> timestamp) {
    ...
    private boolean equalTo(UserCreated another) {
      return userId.equals(another.userId) && name.equals(another.name) 
    		  &&
      		pw.equals(another.pw) && timestamp.equals(another.timestamp);
    }
    ...
    public String toString() {
      return MoreObjects.toStringHelper("UserCreated").add("userId", userId).add("name", name).add("pw", pw).
